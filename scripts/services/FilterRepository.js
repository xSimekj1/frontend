    ...
    angular.module('ngMain').factory('$filterRepository', function ($q, $log, $process, $user, $i18n, $config) {
        ...
        const repository = {
            ...
            createDefaults: function () {
                repository.put($config.show.mycases.viewId, new Filter("Base case filter", Filter.CASE_TYPE, `{"author": {"id": ${$user.id}}}`));
                repository.put("tasks", new Filter($i18n.page.tasks.all, Filter.TASK_TYPE, `{"user":${$user.id}}`, "{}", null, null));
