define(['angular','../classes/Filter', 'angularRoute', '../modules/Main'], function (angular, Filter) {
    angular.module('ngMain').factory('$auth', function ($http, $location, $rootScope, $log, $timeout, $user, $snackbar, $i18n, $process, $config, $filterRepository, $BrowserSupportNotification, $authToken) {
        ...
        const auth = {
            ...
            loadTeam: function (callback = angular.noop) {
                $http({
                    method: 'POST',
                    url: $config.getApiUrl("/user/search"),
                    params: {
                        small: true
                    },
                    data: {
                    ...
                    response.$request().$get("users").then(resources => {
                        $user.teamMembers = resources.map(user => {
                            return user.id
                        });
                        $filterRepository.put("cases", new Filter("Base case filter", Filter.CASE_TYPE, `{"author":  [${$user.teamFilter()}]}`));
                    });
